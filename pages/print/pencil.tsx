...
export default function PrintablePencil() {
  ...
  // Load the snapshot saved by PrintPencilButton
  useEffect(() => {
    try {
  ...
  const fullName = [fmt(customer?.firstName), fmt(customer?.lastName)]
    .filter(Boolean)
    .join(" ");
  const address = [fmt(customer?.address), [fmt(customer?.city), fmt(customer?.state), fmt(customer?.zip)]
    .filter(Boolean)
    .join(" ")]
    .filter(Boolean)
    .join(", ");
  const vehicleLine = [fmt(deal?.year), fmt(deal?.make), fmt(deal?.model)]
    .filter(Boolean)
    .join(" ");
    ...
    <main className="sheet">
      <header className="hdr">
        ...
        <div className="meta">
          ...
          {_meta?.savedAt && (
            <div>
              <b>Saved:</b> {new Date(_meta.savedAt).toLocaleString()}
            </div>
          )}
        </div>
      </header>
      ...
      <section className="grid">
        <div className="card">
          ...
          <div className="row">
            <label>Mobile</label>
            <span>{fmt(customer?.cell) || "—"}</span>
          </div>
          <div className="row">
            <label>Email</label>
            <span>{fmt(customer?.email) || "—"}</span>
          </div>
          <div className="row">
            <label>Address</label>
            <span>{address || "—"}</span>
        ...
        <div className="card">
          ...
          <div className="row">
            <label>VIN</label>
            <span>{fmt(deal?.vin) || "—"}</span>
          </div>
          <div className="row">
            <label>Stock</label>
            <span>{fmt(deal?.stock) || "—"}</span>
          </div>
          <div className="row">
            <label>Type</label>
            <span>{fmt(deal?.newOrUsed) || "—"}</span>
          </div>
        </div>
      </section>
      ...
      <section className="sign">
        ...
        <div className="sig">
          <div className="line" /> Co-Buyer
        </div>
        <div className="sig">
          <div className="line" /> Salesperson
        </div>
        <div className="sig">
